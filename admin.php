...
if ( !function_exists( 'zodan_onetime_login_link_register_settings' ) ) {
    ...
    function zodan_onetime_login_link_register_settings() {
		...
		add_settings_field(
			...
			esc_html__('Excluded roles', 'zodan-onetime-login-link') . '<span class="description">' .
                esc_html__( 'Some user roles should not have a fast login', 'zodan-onetime-login-link' ) . '</span>', 
		...
		add_settings_field(
			...
			esc_html__('Login link text', 'zodan-onetime-login-link'), 
		...
		add_settings_field(
			...
			esc_html__('Mail subject', 'zodan-onetime-login-link'), 
		...
		add_settings_field(
			...
			esc_html__('Mail content', 'zodan-onetime-login-link'), 
		...
		add_settings_field(
			...
			esc_html__('Link expiration time', 'zodan-onetime-login-link'), 
		...
		add_settings_field(
			...
			esc_html__('User request', 'zodan-onetime-login-link'), 
		...
		add_settings_field(
			...
			esc_html__('Use rate limiting', 'zodan-onetime-login-link'), 
		...
		add_settings_field(
			...
			esc_html__('Rate limit', 'zodan-onetime-login-link'), 
		...
		add_settings_field(
			...
			esc_html__('Log bulk mailings', 'zodan-onetime-login-link'), 
    ...
    function zodan_onetime_login_link_roles_section_text() { 
        ...
        echo '<summary><h2>';
        esc_html_e( 'Roles', 'zodan-onetime-login-link' );
        echo '</h2></summary><div class="details-content">';

        echo '<p><strong class="z-warning zotll-warning">' . esc_html__('Please note', 'zodan-onetime-login-link') . '</strong> ';
        echo esc_html__("While we've prioritized security in developing this plugin, we still recommend against using direct login for certain roles.", 'zodan-onetime-login-link');
        echo '<br>';
        echo esc_html__('Be wise and exclude roles like Administrators, Editors, Shop managers.', 'zodan-onetime-login-link');
        echo '</p>';     
    }   
    ...
    function zodan_onetime_login_link_mail_section_text() {
        echo '<summary><h2>';
        esc_html_e('Mail settings', 'zodan-onetime-login-link');
        echo '</h2></summary><div class="details-content">';
    }   
    ...
    function zodan_onetime_login_link_render_mail_linktext() {
        ...
        $mail_linktext = isset( $options['mail_linktext'] ) ? $options['mail_linktext'] : __('Click here to login without a password', 'zodan-onetime-login-link');
        ...
        printf('<label><strong class="screen-reader-text">%s:</strong> <input type="text" id="zmail-linktext" name="zodan_onetime_login_link_plugin_options[mail_linktext]" value="%s"> <span id="zlinktext-example-frame"> %s: <a href="#" id="zlinktext-example"></a></span></label><br>%s.',
            esc_html(__('Text', 'zodan-onetime-login-link')),
            esc_html( $mail_linktext),
            esc_html(__('resulting in something like', 'zodan-onetime-login-link')),
            esc_html(__('If you leave this blank, the link text will display the URL', 'zodan-onetime-login-link')),
        );
    }   
    ...
    function zodan_onetime_login_link_render_mail_subject() {
        ...
        $mail_subject = isset( $options['mail_subject'] ) ? $options['mail_subject'] : __('Your One-time Login Link', 'zodan-onetime-login-link');
        ...
        printf('<label><strong class="screen-reader-text">%s:</strong> <input type="text" name="zodan_onetime_login_link_plugin_options[mail_subject]" value="%s"></label>',
            esc_html(__('Subject', 'zodan-onetime-login-link')),
    ...
    function zodan_onetime_login_link_render_mail_content() {
		...
		$default_mail_content = '<p>'.__('Hello {{firstname}}', 'zodan-onetime-login-link').'</p>';
        $default_mail_content .= '<p>'.__('With the following link you can directly log in without having to enter your password.', 'zodan-onetime-login-link').'</p>';
        $default_mail_content .= '<p>{{zloginlink}}</p><p>'.__('This link can be used only once.', 'zodan-onetime-login-link').'</p>';
        ...
        echo '<p>';
        // echo esc_html(__("Enter the mail content that will be sent to the user.", 'zodan-onetime-login-link')).'<br>';
        echo esc_html(__("The text must include the {{zloginlink}} code where the actual login link should be included.", 'zodan-onetime-login-link')).'<br>';
        echo esc_html(__("You can also include the {{displayname}} and {{firstname}} codes for the user's display name and first name respectively.", 'zodan-onetime-login-link')).'</p>';
    ...
    function zodan_onetime_login_link_other_options_section_text() {
        echo '<summary><h2>';
        esc_html_e('Other options', 'zodan-onetime-login-link');
        echo '</h2></summary><div class="details-content">';
    }   
    ...
    function zodan_onetime_login_link_render_user_request_checkbox() {
        ...
        printf(
            ...
            esc_html( __('Allow users to request a new One-time Login Link on the login form, similar to the reset password method.', 'zodan-onetime-login-link') )
        );
    }   
    ...
    function zodan_onetime_login_link_render_token_expire_time() {
        ...
        printf('<label><strong class="screen-reader-text">%s:</strong> <input type="number" name="zodan_onetime_login_link_plugin_options[expire_time]" value="%s" min="300"> %s</label>',
            ...
            esc_html(__('seconds', 'zodan-onetime-login-link')),
        );
    }   
    ...
    function zodan_onetime_login_link_render_rate_limit_checkbox() {
        ...
        printf(
            ...
            esc_html( __('Use rate limiting to the user requests (e.g. max. once every 10 minutes)', 'zodan-onetime-login-link') )
        );
    }   
    ...
    function zodan_onetime_login_link_render_rate_limit_value() {
        ...
        printf('<label><strong class="screen-reader-text">%s:</strong> <input type="number" id="z_rate_limit_value" name="zodan_onetime_login_link_plugin_options[rate_limit_value]" value="%s" min="60"> %s</label>',
            ...
            esc_html(__('seconds between requests', 'zodan-onetime-login-link')),
        );
    }   
    ...
    function zodan_onetime_login_link_render_bulk_mail_logging() {
        ...
        printf(
            ...
            esc_html( __('Log bulk mail activities (number of users processed, scheduled cron jobs etc.)', 'zodan-onetime-login-link') )
        );
    ...
    function zodan_onetime_login_link_add_admin_menu() {
        add_options_page(
            __('Login once', 'zodan-onetime-login-link'),
            'Login Once',
            'manage_options',
    ...
    function zodan_onetime_login_link_options_page() {
        ...
        ?>
        <div class="wrap">
            <h1><?php esc_html_e('Zodan One-time Login Link settings', 'zodan-onetime-login-link'); ?></h1>
            <form action="options.php" method="post">
                <?php
                settings_fields( 'zodan_onetime_login_link_plugin_options' );

                echo '<h2>' . esc_html__('Introduction', 'zodan-onetime-login-link') . '</h2>';
                echo '<p>' . esc_html__('With the One-time Login Link you can give users the option to log into Wordpress without a password.', 'zodan-onetime-login-link') . '</p>';
                echo '<p>' . esc_html__('Current version', 'zodan-onetime-login-link') . ': '.esc_html( zodan_oneTIME_LOGIN_LINK_VERSION ).'</p>';
                
                $users_url = admin_url( 'users.php');
                echo '<p><span class="dashicons dashicons-email-alt"></span> '.esc_html__( 'You can send a One-time Login Link to ALL active users from the', 'zodan-onetime-login-link' ). ' ';
                printf('<a href="%s">%s</a>.',
                    esc_url( $users_url ),
                    esc_html__( 'users admin page', 'zodan-onetime-login-link' )
                );
                echo '</p>';
                echo '<p>&nbsp;</p>';
    ...
    function zodan_onetime_login_link_admin_footer_print_thankyou( $data ) {
        $data = '<p class="zThanks"><a href="https://zodan.nl" target="_blank" rel="noreferrer">' .
                esc_html__('Made with', 'zodan-onetime-login-link') . 
                ...
                esc_html__('by Zodan', 'zodan-onetime-login-link') .
                '</a></p>';
